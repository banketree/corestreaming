...
public class EncoderDebugger {
	...
	/** Modified: encode elapsed time */
	private int encodeElapsed = 5000000;

	/** Modified: decode elapsed time */
	private int decodeElapsed = 15000000;

//	/** Modified: encode elapsed time upper limit */
//	private final static int ENCODE_ELAPSED_UL = 6000000;
//
//	/** Modified: decode elapsed time upper limit */
//	private final static int DECODE_ELAPSED_UL = 6000000;
	...
	private void debug() {
		...
		//Modified: check the main manufacture cpu encoder
		//temporarily comment
//		String cpuName = DeviceInfoUtils.getCpuName(DeviceInfoUtils.getCpuInfo());
//		if (DeviceInfoUtils.isMtkCpu(cpuName)) {
//			reset();
//			mEncoderName = "OMX.MTK.VIDEO.ENCODER.AVC";
//			mEncoderColorFormat = 19;
//		}
//		if (DeviceInfoUtils.isSamsungCpu(cpuName)) {
//			for (Codec tempCodec : encoders) {
//				if (tempCodec.name.equals("OMX.Exynos.AVC.Encoder")) {
//					reset();
//					mEncoderName = "OMX.Exynos.AVC.Encoder";
//					for (int j = 0; j < tempCodec.formats.length; j++) {
//						if (tempCodec.formats[j].intValue() == 19) {
//							mEncoderColorFormat = 19;
//							break;
//						} else if (tempCodec.formats[j].intValue() == 21) {
//							mEncoderColorFormat = 21;
//							break;
//						}
//					}
//					break;
...
//			}
//		}
//		if (DeviceInfoUtils.isQualcommCpu(cpuName)) {
//			for (Codec tempCodec : encoders) {
//				if (tempCodec.name.equals("OMX.qcom.video.encoder.avc")) {
//					reset();
//					mEncoderName = "OMX.qcom.video.encoder.avc";
//					mEncoderColorFormat = 21;
//					break;
//				} else if (tempCodec.name.equals("OMX.brcm.video.h264.hw.encoder")) {
//					reset();
//					mEncoderName = "OMX.brcm.video.h264.hw.encoder";
//					mEncoderColorFormat = 19;
//					break;
//				}
//			}
//		}
		...
		for (Codec tempCodec : encoders) {
			if (tempCodec.name.equals("OMX.qcom.video.encoder.avc")) {
				...
				mEncoderColorFormat = 21;
				break;
			} else if (tempCodec.name.equals("OMX.MTK.VIDEO.ENCODER.AVC")) {
				reset();
				mEncoderName = "OMX.MTK.VIDEO.ENCODER.AVC";
				mEncoderColorFormat = 19;
				break;
			} else if (tempCodec.name.equals("OMX.Exynos.AVC.Encoder")) {
				reset();
				mEncoderName = "OMX.Exynos.AVC.Encoder";
				for (int j = 0; j < tempCodec.formats.length; j++) {
					if (tempCodec.formats[j].intValue() == 19) {
		...
		if (mEncoderName != null) {
			if (encodeDecodeTest(decoders)) {
				return;
			}
		} else {
			...
			for (int i=0;i<encoders.length;i++) {
				for (int j=0;j<encoders[i].formats.length;j++) {
					...
					//Modified: mEncoder.queueInputBuffer(bufferIndex, 0, mData.length, timestamp(), 0); in
					//searchSPSandPPS(); would stuck when use "OMX.ittiam.video.encoder.avc"/"OMX.SEC.avc.sw.enc"
					//to encode the data, this method used to work around it
					if (Config.WORKAROUND_ENCODER_DEBUGGER_FAIL_TO_SEARCH_SPS_AND_PPS &&
							mEncoderName.equals("OMX.ittiam.video.encoder.avc") ||
							mEncoderName.equals("OMX.SEC.avc.sw.enc") ||
							mEncoderName.equals("OMX.brcm.video.h264.encoder")) {
						Log.d(TAG, "Bypass!");
						break;
					}
	...
	//Modified: encode some video frames with the test image data then decode the
	//video data to generate some frames to compare with the original data then
	//set the NV21Convertor corresponding parameters
	private boolean encodeDecodeTest(Codec[] decoders) {
		...
		try {
			...
			if (!decoded) {
				if (Config.WORKAROUND_ENCODER_DEBUGGER_FAIL_TO_DECODE ) {
					Log.d(TAG, "Bypass!");
					return true;
				}
			...
			if (!compareLumaPanes()) {
				//Modified: workaround this exception
				if (Config.WORKAROUND_ENCODER_DEBUGGER_COMPARE_LUMA_PANES ) {
					Log.d(TAG, "Bypass!");
					return true;
				}
			...
			if ((padding = checkPaddingNeeded())>0) {
				if (padding<4096) {
					...
					//Modified: workaround this exception
					if (Config.WORKAROUND_ENCODER_DEBUGGER_CHECK_PADDING ) {
						Log.d(TAG, "Bypass!");
						return true;
					}
			...
			if (!compareChromaPanes(false)) {
				if (compareChromaPanes(true)) {
					...
					//Modified: workaround this exception
					if (Config.WORKAROUND_ENCODER_DEBUGGER_COMPARE_CHROMA_PANES ) {
						Log.d(TAG, "Bypass!");
						return true;
					}
...
//		//Modified: some device need more time to encode the data
//		if (encodeElapsed < ENCODE_ELAPSED_UL) {
//			encodeElapsed += 1000000;
//			encode();
//			return elapsed;
//		}
	...
	private long decode(boolean withPrefix) {
		...
		//Modified: some device need more time to decode the data
		//change elapsed upper limit from 3000000 to 5000000
...
//		//Modified: some device need more time to decode the data
//		if (decodeElapsed < DECODE_ELAPSED_UL) {
//			decodeElapsed += 1000000;
//			decode(true);
//			return elapsed;
//		}
